			...
			if (theView.getRdbtnClientId().isSelected()) {
				...
				if (searchParam != null) {
					try {
						...
						theClient.sendMessage(theMessage);
						//selectedList = theModel.searchByClientType(searchParam.toUpperCase().charAt(0));
						//addToListModel(selectedList);
...
//			} else {
//
//				if (validateUserInput(fName, lName, address, postalCod, phoneNum)) {
...
//			} else {
//				if (validateUserInput(fName, lName, address, postalCod, phoneNum)) {
//					Customer c = theModel.constructClient(fName, lName, address, postalCod, phoneNum, clientType);
